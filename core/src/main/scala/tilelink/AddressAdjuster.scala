...
class AddressAdjuster(
  ...
  private val defaultRegion: AddressSet =
    params.local.copy(base = params.local.base + localBaseAddressDefault.getOrElse(0))
  ...
  localBaseAddressDefault.foreach { x =>
    require((x & ~mask) == 0, s"localBaseAddressDefault $localBaseAddressDefault was not aligned to region")
  }
  forceLocal.foreach { as =>
    require((as.max & mask) == 0, s"forceLocal $forceLocal must be contained within the lowest index region")
  }
  ...
  val node: TLJunctionNode = TLJunctionNode(
    ...
    managerFn = { mp =>
      ...
      val newRemotes = adjustableRemoteManagers.map { r =>
        r.v1copy(address = prefixNotDefault(r.address), fifoId = if (ordered) Some(0) else None)
      }
    ...
    node.inoutGrouped.collect {
      case (Seq((parent, parentEdge)), Seq((local, localEdge), (remote, remoteEdge))) =>
        ...
        def isDynamicallyLocal(addr: UInt) =
          (local_prefix & mask.U) === (addr & mask.U) || containsAddress(forceLocal, addr)
