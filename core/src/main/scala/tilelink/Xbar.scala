...
object TLXbar {
  def circuit(policy: TLArbiter.Policy, seqIn: Seq[(TLBundle, TLEdge)], seqOut: Seq[(TLBundle, TLEdge)]): Unit = {
    ...
    (seqOut.map(_._1) zip seqOut_ACancel.map(_._1)) foreach {
      case (sOut, sOut_ACancel) =>
...
object TLXbar_ACancel {
  def circuit(
    ...
    seqIn: Seq[(TLBundle_ACancel, TLEdge)],
    seqOut: Seq[(TLBundle_ACancel, TLEdge)]
  ): Unit = {
    ...
    val portsAOI = transpose((in zip requestAIO) map {
      case (i, r) => TLXbar_ACancel.fanout(i.a, r, edgesOut.map(_.params(ForceFanoutKey).a))
    })
  ...
  // Replicate an input port to each output port
  def fanout[T <: TLChannel](
    input: ReadyValidCancel[T],
    select: Seq[Bool],
    force: Seq[Boolean] = Nil
  ): Seq[ReadyValidCancel[T]] = {
    val filtered = Wire(Vec(select.size, chiselTypeOf(input)))
    for (i <- 0 until select.size) {
      filtered(i).bits := (if (force.lift(i).getOrElse(false)) IdentityModule(input.bits) else input.bits)
