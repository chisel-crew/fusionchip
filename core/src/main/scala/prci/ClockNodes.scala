...
case class ClockSourceNode(val portParams: Seq[ClockSourceParameters])(implicit valName: ValName)
  ...
  def fixedClockResources(name: String, prefix: String = "soc/"): Seq[Option[FixedClockResource]] = portParams.map {
    p => p.give.map(g => new FixedClockResource(name, g.freqMHz, prefix))
  }
}
...
case class ClockSinkNode(val portParams: Seq[ClockSinkParameters])(implicit valName: ValName)
  ...
  def fixedClockResources(name: String, prefix: String = "soc/"): Seq[Option[FixedClockResource]] = portParams.map {
    p => p.take.map(t => new FixedClockResource(name, t.freqMHz, prefix))
  }
}

case class ClockAdapterNode(sourceFn: ClockSourceParameters => ClockSourceParameters = { m => m }, sinkFn: ClockSinkParameters => ClockSinkParameters = {
  s => s
...
case class ClockGroupAdapterNode(sourceFn: ClockGroupSourceParameters => ClockGroupSourceParameters = { m => m }, sinkFn: ClockGroupSinkParameters => ClockGroupSinkParameters = {
  s => s
})(implicit valName: ValName)
