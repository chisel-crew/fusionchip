...
import chisel3._
import chisel3.util.{ isPow2, log2Ceil, log2Up, Cat, OHToUInt, Queue, QueueIO, UIntToOH }
...
class AXI4Deinterleaver(maxReadBytes: Int)(implicit p: Parameters) extends LazyModule {
  ...
  val node = AXI4AdapterNode(
    ...
    slaveFn = { sp =>
      sp.copy(slaves =
        sp.slaves.map(s =>
          s.copy(supportsRead = s.supportsRead.intersect(TransferSizes(1, maxReadBytes)), interleavedId = Some(0))
        )
      )
    }
  )
