...
class AXISBundleParameters private (
  ...
  def union(x: AXISBundleParameters) =
...
object AXISBundleParameters {
  val emptyBundleParams =
    new AXISBundleParameters(idBits = 0, destBits = 0, dataBits = 8, userFields = Nil, oneBeat = true, aligned = true)
...
class AXISEdgeParameters private (
  ...
  def v1copy(
    ...
    new AXISEdgeParameters(master = master, slave = slave, params = params, sourceInfo = sourceInfo)
}

object AXISEdgeParameters {
  def v1(master: AXISMasterPortParameters, slave: AXISSlavePortParameters, params: Parameters, sourceInfo: SourceInfo) =
    new AXISEdgeParameters(master = master, slave = slave, params = params, sourceInfo = sourceInfo)
}
