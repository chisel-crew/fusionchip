...
class LanePositionedQueueBase[T <: Data](val gen: T, args: LanePositionedQueueArgs)
    extends Module
...
class OnePortLanePositionedQueueModule[T <: Data](ecc: Code)(gen: T, args: LanePositionedQueueArgs)
  ...
  val ram_i        = Mux(write_row(0), VecInit(enq_buffer(2) ++ enq_buffer(3)), VecInit(enq_buffer(0) ++ enq_buffer(1)))
  val read_row =
    Mux(
  ...
  val dlatch0 = RegNext(
    (deq_push && !deq_row(1)) || (deq_refill_ren1 && deq_row(1)) || (deq_refill_ren0 && !deq_row(1))
  )
