...
class ICacheModule(outer: ICache) extends LazyModuleImp(outer) with HasL1ICacheParameters {
  ...
  val s2_report_uncorrectable_error =
    s2_scratchpad_hit && s2_data_decoded.uncorrectable && (s2_valid || (s2_slaveValid && !s2_full_word_write))
  val s2_error_addr =
  ...
  tl_out.a.bits := edge_out
    .Get(fromSource = UInt(0), toAddress = (refill_paddr >> blockOffBits) << blockOffBits, lgSize = lgCacheBlockBytes)
    ._2
