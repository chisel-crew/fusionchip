...
class L2TLBEntry(implicit p: Parameters) extends CoreBundle()(p) with HasCoreParameters {
...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val clock_en =
    state =/= s_ready || arb.io.out.valid || io.dpath.sfence.valid || io.dpath.customCSRs.disableDCacheClockGate
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    val fragmented_superpage_ppn = {
      val choices = (pgLevels - 1 until 0 by -1).map(i =>
        Cat(r_pte.ppn >> (pgLevelBits * i), r_req.addr(((pgLevelBits * i) min vpnBits) - 1, 0).padTo(pgLevelBits * i))
      )
      choices(count)
    }
