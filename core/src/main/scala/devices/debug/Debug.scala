...
object RWNotify {
  def apply(n: Int, rVal: UInt, wVal: UInt, rNotify: Bool, wNotify: Bool, desc: Option[RegFieldDesc] = None): RegField =
    RegField(n, RegReadFn { (ready) => rNotify := ready; (true.B, rVal) }, RegWriteFn { (valid, data) =>
      wNotify := valid
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val omRegMap = withReset(reset.asAsyncReset) {
      ...
      val dmAuthenticated = io.dmAuthenticated
        .map(dma => ResetSynchronizerShiftReg(in = dma, sync = 3, name = Some("dmAuthenticated_sync")))
        .getOrElse(true.B)
...
class TLDebugModuleOuterAsync(device: Device)(implicit p: Parameters) extends LazyModule {
  ...
  val apbNodeOpt = p(ExportDebug).apb.option({
    ...
    val tlErrorParams = DevNullParams(
      AddressSet.misaligned(dmTopAddr, APBDebugConsts.apbDebugRegBase - dmTopAddr),
      maxAtomic = 0,
      maxTransfer = 4
    )
  ...
  lazy val module = new LazyRawModuleImp(this) {
    ...
    withClockAndReset(childClock, childReset) {
      ...
      val dmactiveAck =
        AsyncResetSynchronizerShiftReg(in = io.ctrl.dmactiveAck, sync = 3, name = Some("dmactiveAckSync"))
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    if (nHaltGroups > 0)
      withReset(reset.asAsyncReset) { // async reset ensures triggers don't falsely fire during startup
        ...
        io.extTrigger.foreach { extTrigger =>
          ...
          val trigInReq =
            ResetSynchronizerShiftReg(in = extTriggerInReq, sync = 3, name = Some("dm_extTriggerInReqSync"))
          val trigOutAck =
            ResetSynchronizerShiftReg(in = extTriggerOutAck, sync = 3, name = Some("dm_extTriggerOutAckSync"))
    ...
    tlNode.regmap(
      ...
      FLAGS -> RegFieldGroup(
        ...
        if (nComponents == 1) {
          Seq.tabulate(1024) { i =>
            RegField.r(8, flags(0).asUInt(), RegFieldDesc(s"debug_flags_$i", "", volatile = true))
          }
        } else {
    ...
    val accessRegIsCustom = if (needCustom) {
      val (custom, customP) = customNode.in.head
      customP.addrs.foldLeft(false.B)((result, current) => result || (current.U === accessRegisterCommandReg.regno))
    } else false.B
...
class TLDebugModule(beatBytes: Int)(implicit p: Parameters) extends LazyModule {
  ...
  val device = new SimpleDevice("debug-controller", Seq("sifive,debug-013", "riscv,debug-013")) {
    ...
    override def describe(resources: ResourceBindings): Description = {
      ...
      val attach = Map(
        "debug-attach" -> ((if (p(ExportDebug).apb) Seq(ResourceString("apb")) else Seq()) ++
  ...
  val dmInner: TLDebugModuleInnerAsync = LazyModule(
    new TLDebugModuleInnerAsync(device, () => dmOuter.dmOuter.intnode.edges.out.size, beatBytes)(p)
  )
