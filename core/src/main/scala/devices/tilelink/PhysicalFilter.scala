...
class PhysicalFilter(params: PhysicalFilterParams)(implicit p: Parameters) extends LazyModule {
  val node = TLAdapterNode(managerFn = { mp =>
    mp.v1copy(managers = mp.managers.map(_.v1copy(alwaysGrantsT = false)), endSinkId = if (mp.endSinkId == 0) { 0 }
    else { mp.endSinkId + 1 }, minLatency = 1 min mp.minLatency)
  })
    ...
    (node.in zip node.out) foreach {
      case ((in, edgeIn), (out, edgeOut)) =>
        ...
        if (edgeIn.manager.anySupportAcquireB) {
          ...
          edgeIn.client.unusedSources.foreach(id => wSourceVec(id) := Bool(true))
