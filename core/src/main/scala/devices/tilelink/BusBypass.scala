...
abstract class TLBusBypassBase(
  ...
  protected val bar = LazyModule(new TLBusBypassBar(dFn = { mp =>
    mp.v1copy(managers = mp.managers.map { m =>
      m.v1copy(mayDenyPut = m.mayDenyPut || !deadlock, mayDenyGet = m.mayDenyGet || !deadlock)
    })
  }))
...
class TLBusBypass(beatBytes: Int, bufferError: Boolean = false, maxAtomic: Int = 16, maxTransfer: Int = 4096)(
  implicit p: Parameters
...
class TLBypassNode(dFn: TLSlavePortParameters => TLSlavePortParameters)(implicit valName: ValName)
