...
case class TileMasterPortParams(buffers: Int = 0, cork: Option[Boolean] = None, where: TLBusWrapperLocation = SBUS)
...
trait HasRocketTiles extends HasTiles with CanHavePeripheryPLIC with CanHavePeripheryCLINT with HasPeripheryDebug {
  ...
  rocketTiles.map(r => LogicalModuleTree.add(logicalTreeNode, r.rocketLogicalTree))
...
class RocketSubsystem(implicit p: Parameters) extends BaseSubsystem with HasRocketTiles {
  ...
  val hartPrefixes = hartPrefixNode.map { hpn =>
    Seq.fill(tiles.size) {
