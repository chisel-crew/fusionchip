...
object Property {
  ...
  def apply(dir: MonitorDirection, cond: Seq[Seq[Bool]], crosscond: Bool, message: String, prop_type: PropertyClass)(
    implicit sourceInfo: SourceInfo
  ): Unit =
...
object OneHotPriorityEncoder {
  ...
  def apply(in: Bits): Bits =
    if (in.getWidth == 1) { in }
      ...
      chisel3.util
        .Mux1H(chisel3.util.UIntToOH(in(0) === true.B), Seq(Cat(apply(in >> 1), 0.U(1.W)), 1.U(in.getWidth.W)))
    }
}
