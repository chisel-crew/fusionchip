...
class SPIFlashTopModule(c: SPIFlashParamsBase, outer: TLSPIFlashBase) extends SPITopModule(c, outer) {
  ...
  val regmapFlash = Seq(
    ...
    SPICRs.insnfmt -> RegFieldGroup(
      ...
      Seq(
        ...
        RegField(c.insnPadLenBits, insn.pad.cnt, RegFieldDesc("pad_cnt", "Number of dummy cycles", reset = Some(0)))
      )
    ),
    ...
    SPICRs.insncmd -> RegFieldGroup(
      "ffmt",
      None,
      Seq(RegField(c.insnCmdBits, insn.cmd.code, RegFieldDesc("cmd_code", "Value of command byte", reset = Some(3))))
    ),
    ...
    def getOMComponents(resourceBindings: ResourceBindings, children: Seq[OMComponent] = Nil): Seq[OMComponent] =
      Seq(
        OMSPIXIP(
          ...
          memoryRegions =
            DiplomaticObjectModelAddressing.getOMMemoryRegions("SPIXIP", resourceBindings, Some(module.omRegMap)),
