...
import sifive.blocks.util.DeviceParams
import sifive.blocks.devices.pinctrl.{ EnhancedPin, EnhancedPinCtrl }
import sifive.blocks.util.DeviceAttachParams
...
case class GPIOParams(address: BigInt, width: Int, includeIOF: Boolean = false) extends DeviceParams
...
abstract class GPIO(busWidthBytes: Int, c: GPIOParams)(implicit p: Parameters)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val iofEnFields =
      if (c.includeIOF)(Seq(
        RegField.rwReg(c.width, iofEnReg.io, Some(RegFieldDesc("iof_en", "HW I/O functon enable", reset = Some(0))))
      ))
      else (Seq(RegField(c.width)))
    val iofSelFields =
      if (c.includeIOF)(Seq(
        RegField(c.width, iofSelReg, RegFieldDesc("iof_sel", "HW I/O function select", reset = Some(0)))
      ))
      else (Seq(RegField(c.width)))
    ...
    val mapping = Seq(
      GPIOCtrlRegs.value -> Seq(
        RegField.r(c.width, valueReg, RegFieldDesc("input_value", "Pin value", volatile = true))
      ),
      GPIOCtrlRegs.output_en -> Seq(
        RegField.rwReg(c.width, oeReg.io, Some(RegFieldDesc("output_en", "Pin output enable", reset = Some(0))))
      ),
      GPIOCtrlRegs.rise_ie -> Seq(
        RegField(c.width, riseIeReg, RegFieldDesc("rise_ie", "Rise interrupt enable", reset = Some(0)))
      ),
      GPIOCtrlRegs.rise_ip -> Seq(
        RegField
          .w1ToClear(c.width, riseIpReg, rise, Some(RegFieldDesc("rise_ip", "Rise interrupt pending", volatile = true)))
      ),
      GPIOCtrlRegs.fall_ie -> Seq(
        RegField(c.width, fallIeReg, RegFieldDesc("fall_ie", "Fall interrupt enable", reset = Some(0)))
      ),
      GPIOCtrlRegs.fall_ip -> Seq(
        RegField
          .w1ToClear(c.width, fallIpReg, fall, Some(RegFieldDesc("fall_ip", "Fall interrupt pending", volatile = true)))
      ),
      GPIOCtrlRegs.high_ie -> Seq(
        RegField(c.width, highIeReg, RegFieldDesc("high_ie", "High interrupt enable", reset = Some(0)))
      ),
      GPIOCtrlRegs.high_ip -> Seq(
        RegField.w1ToClear(
          c.width,
          highIpReg,
          valueReg,
      ...
      GPIOCtrlRegs.low_ie -> Seq(
        RegField(c.width, lowIeReg, RegFieldDesc("low_ie", "Low interrupt enable", reset = Some(0)))
      ),
      GPIOCtrlRegs.low_ip -> Seq(
        RegField.w1ToClear(
          c.width,
          lowIpReg,
          ~valueReg,
      ...
      GPIOCtrlRegs.port -> Seq(
        RegField(c.width, portReg, RegFieldDesc("output_value", "Output value", reset = Some(0)))
      ),
      GPIOCtrlRegs.pullup_en -> Seq(
        RegField.rwReg(c.width, pueReg.io, Some(RegFieldDesc("pue", "Internal pull-up enable", reset = Some(0))))
      ),
      GPIOCtrlRegs.iof_en  -> iofEnFields,
      GPIOCtrlRegs.iof_sel -> iofSelFields,
      GPIOCtrlRegs.drive -> Seq(
        RegField(c.width, dsReg, RegFieldDesc("ds", "Pin drive strength selection", reset = Some(0)))
      ),
      GPIOCtrlRegs.input_en -> Seq(
        RegField.rwReg(c.width, ieReg.io, Some(RegFieldDesc("input_en", "Pin input enable", reset = Some(0))))
      ),
      GPIOCtrlRegs.out_xor -> Seq(
        RegField(c.width, xorReg, RegFieldDesc("out_xor", "Output XOR (invert) enable", reset = Some(0)))
      ),
      GPIOCtrlRegs.passthru_high_ie -> Seq(
        RegField(
          c.width,
          passthruHighIeReg,
      ...
      GPIOCtrlRegs.passthru_low_ie -> Seq(
        RegField(
          c.width,
          passthruLowIeReg,
    ...
    def getOMComponents(resourceBindings: ResourceBindings, children: Seq[OMComponent] = Nil): Seq[OMComponent] =
      Seq(
        OMGPIO(
          ...
          nPins = c.width,
          memoryRegions =
            DiplomaticObjectModelAddressing.getOMMemoryRegions("GPIO", resourceBindings, Some(module.omRegMap)),
...
object GPIO {
  ...
  def tieoff(g: GPIOPortIO) {
    g.pins.foreach(p => p.i.ival := false.B)
    g.iof_0.foreach(i0 => i0.foreach(iof => iof.default()))
    g.iof_1.foreach(i1 => i1.foreach(iof => iof.default()))
  }
  ...
  def loopback(g: GPIOPortIO)(pinA: Int, pinB: Int) {
    ...
    g.pins.foreach(p => p.i.ival := Mux(p.o.oe, p.o.oval, p.o.pue) & p.o.ie)
    val a = g.pins(pinA)
    val b = g.pins(pinB)
