...
class PWMTimer(val ncmp: Int = 4, val cmpWidth: Int = 16, val prefix: String = "pwm")
    extends MultiIOModule
    with GenericTimer {

  def orR(v: Vec[Bool]): Bool = v.foldLeft(Bool(false))(_ || _)
  protected def countWidth    = ((1 << scaleWidth) - 1) + cmpWidth
  protected lazy val countAlways =
    RegEnable(io.regs.cfg.write.countAlways, Bool(false), io.regs.cfg.write_countAlways && unlocked)
  ...
  override protected lazy val extra: Vec[Bool] =
    RegEnable(io.regs.cfg.write.extra, init = Vec.fill(maxcmp)(false.B), orR(io.regs.cfg.write_extra) && unlocked)
  override protected lazy val center: Vec[Bool] =
    RegEnable(io.regs.cfg.write.center, orR(io.regs.cfg.write_center) && unlocked)
  override protected lazy val gang: Vec[Bool] =
    RegEnable(io.regs.cfg.write.gang, orR(io.regs.cfg.write_gang) && unlocked)
  override protected lazy val deglitch =
    RegEnable(io.regs.cfg.write.deglitch, io.regs.cfg.write_deglitch && unlocked)(0)
  ...
  override protected lazy val cfg_desc = DefaultGenericTimerCfgDescs(prefix, ncmp).copy(
    extra = Seq.tabulate(ncmp) { i =>
      RegFieldDesc(s"${prefix}invert${i}", s"Invert Comparator ${i} Output", reset = Some(0))
    }
  )
...
case class PWMParams(address: BigInt, size: Int = 0x1000, regBytes: Int = 4, ncmp: Int = 4, cmpWidth: Int = 16)
    ...
    def getOMComponents(resourceBindings: ResourceBindings, children: Seq[OMComponent] = Nil): Seq[OMComponent] =
      Seq(
        OMPWM(
          ...
          memoryRegions =
            DiplomaticObjectModelAddressing.getOMMemoryRegions("PWM", resourceBindings, Some(module.omRegMap)),
