    ...
    val sbcsfields = RegFieldGroup(
      ...
      Seq(
        ...
        RegField.r(
          7,
          SBCSRdData.sbasize,
          RegFieldDesc("sbasize", "bits in address", reset = Some(sb2tl.module.edge.bundle.addressBits))
        ),
        WNotifyVal(
          3,
          SBCSRdData.sberror,
...
class SBToTL(implicit p: Parameters) extends LazyModule {
  ...
  val node = TLClientNode(
    Seq(TLMasterPortParameters.v1(clients = Seq(TLMasterParameters.v1("debug")), requestFields = Seq(AMBAProtField())))
  )
