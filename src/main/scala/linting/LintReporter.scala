...
import firrtl.ir._
import firrtl.options.{ Dependency, OptionsException, PreservesAll, RegisteredLibrary, ShellOption }
 ...
 */
final class LintReporter
    extends Transform
    with RegisteredLibrary
  ...
  lazy val options = Seq(
    new ShellOption[String](
      ...
      toAnnotationSeq = {
        case "*" =>
          RunFirrtlTransformAnnotation(this) +: (Linter.lintMap.values.map(RunFirrtlTransformAnnotation(_)).toSeq)
        case other =>
          RunFirrtlTransformAnnotation(this) +: (other.split(',').toSeq.map { s =>
      ...
      helpText =
        s"Enable linting for specified rules, where * is all rules. Available rules: ${Linter.linters.map(l => l.lintName).mkString(",")}.",
    ...
    new ShellOption[String](
      ...
      toAnnotationSeq = { arg: String =>
        val displayOptions =
          arg.split(',').toSeq.foldLeft(DisplayOptions()) {
            (opt, str) =>
              str match {
                case "strict"        => opt.copy(level = "strict")
                case "warn"          => opt.copy(level = "warn")
                case displayTotal(n) => opt.copy(totalLimit = Some(n.toInt))
                case perTotal(lint, n) =>
                  opt.copy(perErrorLimit = opt.perErrorLimit + (Linter.lintMap(lint).lintName -> n.toInt))
