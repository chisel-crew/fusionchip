...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  @chiselName class DCacheModuleImpl extends NoChiselNamePrefix { // entering gated-clock domain
    ...
    val s2_want_victimize =
      Bool(!usingDataScratchpad) && (s2_valid_cached_miss || s2_valid_flush_line || s2_valid_data_error || s2_flush_valid)
    ...
    val block_probe_for_pending_release_ack =
      release_ack_wait && (tl_out.b.bits.address ^ release_ack_addr)(idxMSB, idxLSB) === 0
    ...
    val releaseDataBeat =
      Cat(UInt(0), c_count) + Mux(releaseRejected, UInt(0), s1_release_data_valid + Cat(UInt(0), s2_release_data_valid))
    ...
    val s2_data_word_possibly_uncached =
      Mux(cacheParams.pipelineWayMux && doUncachedResp, s2_uncached_data_word, 0.U) | s2_data_word
    ...
    if (coreParams.useVector) {
      edge.manager.managers.foreach { m =>
        ...
        require(
          !m.supportsPutFull || m.supportsPutPartial,
          "With a vector unit, writable memory must support PutPartial"
        )
      }
    }
