...
case class ClockParameters(freqMHz: Double, dutyCycle: Double = 50) //in percent
{
  require(freqMHz > 0)
  require(0 < dutyCycle)
...
case class ClockGroupSinkParameters(name: String, members: Seq[ClockSinkParameters])

case class ClockGroupBundleParameters(members: Seq[ClockBundleParameters])
...
case class ClockGroupEdgeParameters(
  ...
  val members          = sink.members.map(s => ClockEdgeParameters(sourceParameters, s, params, sourceInfo))
...
case class ClockGroupDriverParameters(
  ...
  def drive(
    node: ClockGroupEphemeralNode
  )(implicit p: Parameters, vn: ValName): ModuleValue[HeterogeneousBag[ClockGroupBundle]] =
